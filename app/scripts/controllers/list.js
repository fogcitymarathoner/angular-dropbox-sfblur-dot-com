...
angular.module('angularS3App')
    .controller('ListCtrl', function ($scope, $http) {
        $scope.files = [];
        // $http.get('http://54.191.47.109:9194/api/v1/list').then(function (response) {
        $http.get('http://s3.sfblur.com/api/v1/list').then(function (response) {
            ...
            for (var file in response.data['files']) {
                ...
                $scope.files.push({"file": response.data['files'][file][0], "date": response.data['files'][file][1], "size": response.data['files'][file][2], "extension": response.data['files'][file][3]});
            }
